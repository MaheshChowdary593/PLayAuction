        ...
        <div className="min-h-screen bg-darkBg text-white p-8 font-sans">
            <div className="max-w-6xl mx-auto">
                <header className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-8 lg:mb-12 gap-4">
                    <div>
                        <h1 className="text-[8px] lg:text-[10px] font-black text-slate-500 uppercase tracking-[0.4em] mb-2">Auction Concluded / Phase 2</h1>
                        <h2 className="text-3xl sm:text-4xl lg:text-6xl font-black italic tracking-tighter uppercase leading-none">
                            Squad <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-400">Selection</span>
                        </h2>
                    </div>
                    <div className="text-left lg:text-right flex lg:flex-col items-center lg:items-end gap-4 lg:gap-0">
                        <div className="text-2xl lg:text-4xl font-black font-mono text-yellow-500 mb-1">{formatTime(timer)}</div>
                        <div className="text-[8px] lg:text-[10px] font-black text-slate-500 uppercase tracking-widest whitespace-nowrap">Time Remaining</div>
                    </div>
                </header>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                            ...
                            {squad.map((entry, idx) => {
                                ...
                                return (
                                    <motion.div
                                        ...
                                        className={`
                                            p-3 lg:p-4 rounded-xl lg:rounded-2xl border cursor-pointer transition-all flex items-center justify-between
                                    ...
                                    >
                                        <div className="flex items-center gap-3 lg:gap-4">
                                            <div className={`w-5 lg:w-6 h-5 lg:h-6 rounded-full border-2 flex items-center justify-center transition-colors ${isSelected ? 'bg-blue-500 border-blue-400' : 'border-white/20'}`}>
                                                {isSelected && <div className="w-1.5 lg:w-2 h-1.5 lg:h-2 bg-white rounded-full"></div>}
                                            </div>
                                            <div>
                                                <div className="font-bold text-xs lg:text-sm flex items-center gap-1">
                                                    {entry.name}
                                                    {entry.isOverseas && <span title="Overseas Player" className="text-[10px] lg:text-xs">✈️</span>}
                                                </div>
                                                <div className="text-[8px] lg:text-[10px] text-slate-500 uppercase font-black tracking-widest">₹{entry.boughtFor}L</div>
                                            </div>
                                        </div>
                                    </motion.div>
                                );
                    ...
                    <div className="space-y-6">
                        <div className="glass-card p-6 lg:p-8 rounded-[24px] lg:rounded-[32px] border-white/10 lg:sticky lg:top-8">
                            <h3 className="text-base lg:text-lg font-black uppercase tracking-widest mb-4 lg:mb-6">Selection Method</h3>

                            <button
                                onClick={handleAutoSelect}
                                disabled={isConfirmed || isAutoSelecting}
                                className={`w-full py-3 lg:py-4 rounded-xl font-black uppercase tracking-widest text-[10px] lg:text-xs mb-3 lg:mb-4 transition-all flex items-center justify-center gap-2
                            ...
                            <p className="text-[9px] lg:text-[10px] text-slate-500 font-bold leading-relaxed mb-6 lg:mb-8 px-2 text-center lg:text-left">
                                * AI uses real-world player stats, T20 logic, and tactical balance to pick your optimal 15.
                            </p>

                            <div className="h-px bg-white/5 mb-6 lg:mb-8"></div>

                            <button
                                onClick={handleManualSubmit}
                                disabled={isConfirmed || isAutoSelecting || (selectedIds.length < 15 && squad.length >= 15)}
                                className={`w-full py-3 lg:py-4 rounded-xl font-black uppercase tracking-widest text-[10px] lg:text-xs transition-all
